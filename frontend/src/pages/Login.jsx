    ...
    <div className="min-h-screen flex bg-[var(--bg)]">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          <div className="label-caps mb-3">{mode === "register" ? "Create account" : "Welcome back"}</div>
          <h1 className="font-heading text-4xl sm:text-5xl font-black tracking-tighter leading-none mb-3">
            {mode === "register" ? "Sign up" : "Sign in"}
          </h1>
          <p className="text-[var(--text-secondary)] mb-8 text-sm leading-relaxed">
            {mode === "register"
              ? "Use email & password, or continue with Google. MFA can be added in Settings."
          ...
          <form onSubmit={submit} className="space-y-3 mb-4">
            {mode === "register" && (
              <div>
                ...
                <input data-testid="auth-name-input" required value={name} onChange={(e) => setName(e.target.value)} className="input-flat" placeholder="Your name" />
              </div>
            )}
            <div>
              <label className="label-caps block mb-2">Email</label>
              <input data-testid="auth-email-input" required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input-flat" placeholder="you@example.com" />
            </div>
            <div>
          ...
          <div className="text-center text-sm text-[var(--text-secondary)] mb-4">
            ...
            <button
              ...
              className="underline underline-offset-4 font-medium text-[var(--text)]"
            >
          ...
          <button
            ...
            className="w-full btn-secondary flex items-center justify-center gap-3 py-3 text-base"
          >
